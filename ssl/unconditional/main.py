...
211229: naive version semi-supervised learning (= base-line)
'''
#%%
import argparse
import os
...
def train(datasetL, datasetU, model, optimizer, optimizer_nf, epoch, args, num_classes):
    ...
    for batch_num in progress_bar:
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            z, c, prob, xhat = model.ae(imageU, training=True) # unlabeled
        ...
        loss_avg(loss)
        nf_loss_avg(nf_loss)
        _, _, probL, _ = model.ae(imageL, training=False)
    ...
    if epoch % args['reconstruct_freq'] == 0:
        generate_and_save_images(model, imageU[0][tf.newaxis, ...], num_classes, epoch, f'logs/{args["dataset"]}/{current_time}')
