...
#%%
'''triaining log file'''
sys.stdout = open("./assets/{}/log2_{}.txt".format(PARAMS['data'], date_time), "w")
...
# test_loss = K.metrics.Mean('test_loss', dtype=tf.float32)
...
def supervised_loss(outputs, x_batch_L, y_batch_L, PARAMS):
    ...
    loss_supervised = recon_loss / PARAMS['beta'] + PARAMS['lambda1'] * classification_loss 
...
def supervised_train_step(x_batch_L, y_batch_L, PARAMS,
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        supervised_losses = supervised_loss(supervised_outputs, x_batch_L, y_batch_L, PARAMS)
...
#%%
# @tf.function
def unsupervised_loss(outputs, x_batch, PARAMS):
    eps = 1e-8
    ...
    loss_unsupervised = recon_loss / PARAMS['beta']
...
def unsupervised_train_step(x_batch, PARAMS,
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        unsupervised_losses = unsupervised_loss(unsupervised_outputs, x_batch, PARAMS)
