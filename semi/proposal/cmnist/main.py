...
def main():
    ...
    for epoch in range(args['start_epoch'], args['epochs']):
        ...
        if epoch % args['reconstruct_freq'] == 0:
            loss, recon_loss, info_loss, nf_loss, mixup_z_loss, mixup_xhat_loss, mixup_y_loss, accuracy, sample_recon = train(datasetL, datasetU, model, buffer_model, optimizer, optimizer_nf, epoch, args, num_classes, total_length, test_accuracy_print)
        else:
            loss, recon_loss, info_loss, nf_loss, mixup_z_loss, mixup_xhat_loss, mixup_y_loss, accuracy = train(datasetL, datasetU, model, buffer_model, optimizer, optimizer_nf, epoch, args, num_classes, total_length, test_accuracy_print)
...
def train(datasetL, datasetU, model, buffer_model, optimizer, optimizer_nf, epoch, args, num_classes, total_length, test_accuracy_print):
    ...
    for batch_num in progress_bar:
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            loss_supervised = elbo_lossL + mixup_yL + (lambda_ * (cls_lossL + infoL))
