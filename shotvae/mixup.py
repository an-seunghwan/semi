...
def label_smoothing(image, label, mean, log_sigma, mix_weight):
    ...
    image_shuffle = tf.gather(image, tf.random.shuffle(tf.range(image.shape[0])))
    label_shuffle = tf.gather(label, tf.random.shuffle(tf.range(label.shape[0])))
...
def optimal_match_mix(image, mean, log_sigma, log_prob, mix_weight):
    ...
    for i in range(tf.shape(mean)[0]):
        kl_metric[i, :] = gaussian_kl_divergence(mean, mean.numpy()[i],
                                                log_sigma, log_sigma.numpy()[i]).numpy()
