...
def main():
    ...
    model.load_weights(f'./assets/pretrained/seed{args["seed"]}/model.h5')
    model.summary()
    ...
    for epoch in range(args['start_epoch'], args['epochs']):
        ...
        loss, mixup_loss, rega_loss, regb_loss, accuracy = train(
            datasetL, datasetU, model, buffer_model, pslab_model, optimizer, epoch, args, num_classes, total_length, test_accuracy_print
        )
        ...
        # wandb.log({'(train) mixup_loss': mixup_loss.result().numpy()})
        # wandb.log({'(train) rega_loss': rega_loss.result().numpy()})
        # wandb.log({'(train) regb_loss': regb_loss.result().numpy()})
        # wandb.log({'(train) accuracy': accuracy.result().numpy()})
        
        # wandb.log({'(val) loss': val_loss.result().numpy()})
        # wandb.log({'(val) ce_loss': val_ce_loss.result().numpy()})
        # wandb.log({'(val) rega_loss': val_rega_loss.result().numpy()})
        # wandb.log({'(val) regb_loss': val_regb_loss.result().numpy()})
        # wandb.log({'(val) accuracy': val_accuracy.result().numpy()})
        
        # wandb.log({'(test) loss': test_loss.result().numpy()})
        # wandb.log({'(test) ce_loss': test_ce_loss.result().numpy()})
        # wandb.log({'(test) rega_loss': test_rega_loss.result().numpy()})
        # wandb.log({'(test) regb_loss': test_regb_loss.result().numpy()})
    ...
    # wandb.run.finish()
#%%
def train(datasetL, datasetU, model, buffer_model, pslab_model, optimizer, epoch, args, num_classes, total_length, test_accuracy_print):
